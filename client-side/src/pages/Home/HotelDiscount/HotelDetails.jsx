        ...
        <div>
            <div className='lg:pb-16 pt-24 px-5 sm:px-10 max-w-7xl mx-auto h-auto overflow-hidden'>
                ...
                <div className=' mx-auto'>
                    {
                        hotel.map(h => <div key={h?.id} className='p-3 shadow-xl '>
                            <div className='flex sm:gap-4 flex-col md:flex-row gap-y-3   sm:items-start md:items-center'>
                                <img src={h?.img} alt="" className='lg:h-[230px] xl:h-[300px]' />
                                <div className='space-y-2'>
                                    <h2 className='text-xl font-semibold'>{h?.name}</h2>
                                    <p className='text-lg'>{h?.details}</p>
                                    <p className='font-semibold text-lg'> <span className='font-bold'>Discount:</span> {h?.discount}%</p>
                                    <p className='font-semibold text-lg'> <span className='font-bold'>Cost:</span> ${h?.cost}</p>
                                    <p className='font-semibold text-lg'> <span className='font-bold'>Location:</span> {h?.location}</p>
